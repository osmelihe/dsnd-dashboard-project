...
class LineChart(MatplotlibViz):
    ...
    def visualization(self, entity_id, model, **kwargs):    
        ...
        if df.empty:
            # Create a dummy row so the chart renders safely at 0
            df = pd.DataFrame({'Positive': [0.0], 'Negative': [0.0]}, index=['No Data'])
...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, entity_id, model, **kwargs):
        ...
        if data.empty:
            pred = 0.0 # Default to 0 risk if there is no data available
        else:
            ...
            if model.name == 'team':
                pred = predicted_risk_scores.mean()
                
            # Otherwise set `pred` to the first value
            # of the predict_proba output
            else:
                pred = predicted_risk_scores[0]
